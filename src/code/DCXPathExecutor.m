...
- (id) executeXPath:(NSString *)aXPath errorVar:(NSError **)aErrorVar {
	...
	DHC_LOG(@"Setting current element %@ in list of current elements", document.name);
	[currentNodes addObject:document];

	// Loop through the selectors and execute then.
	DHC_LOG(@"Processing selectors");
	...
	for (DCXPathAbstractSelector *selector in selectors) {
		...
		for (int i = 0; i < [currentNodes count]; i++) {
			...
			if (selectorNodes == nil) {
				DHC_LOG(@"Selector returned nil indicating an error will be present. Stopping xpath processing.");
				return nil;
			}
...
- (NSMutableArray *) selectorPipelineFromXPath:(NSString *)xpath {
	...
	while ([scanner isAtEnd] == NO) {
		...
		switch (nextToken) {
			case ROOT_SLASH:
				DHC_LOG(@"Adding root node selector");
			...
			case ROOT_ELEMENT:
				...
				DHC_LOG(@"Adding filter to root node selector to validate it's name is %@", nodeName);
			...
			case SLASH:
				DHC_LOG(@"Turning off element specification flag.");
			...
			case ELEMENT:
				...
				if (subtreeSearch) {
					DHC_LOG(@"Adding subtree search selector.");
					...
					DHC_LOG(@"Adding element selector for %@", nodeName);
			...
			case NUMBER:
				if (elementSpecification) {
					DHC_LOG(@"Appending filter to element selector for index %i", arrayIndex);
					...
					DHC_LOG(@"Adding index only selector for index %i", arrayIndex);
			...
			case PARENT:
				DHC_LOG(@"Adding parent node selector.");
			...
			case DOT:
				...
				DHC_LOG(@"Self reference, no change to selectors.");
				continue;
...
- (TokenType) tokenTypeFromScanner:(NSScanner *)scanner lastToken:(TokenType)lastToken nodeNameVar:(NSString **)nodeNameVar arrayIndexVar:(int *)arrayIndexVar {
	...
	if ([scanner scanString:@"//" intoString:NULL]) {
		...
		DHC_LOG(@"Found%@ slash (/)", scanStartIndex == 0 ? @" root" : @"");
	} else if ([scanner scanString:@"[" intoString:NULL]) {
		...
		DHC_LOG(@"Found%@ element name", lastToken == ROOT_SLASH ? @" root" : @"");
...
- (void) dealloc {
	DHC_DEALLOC(validElementnameCharacters);
	[super dealloc];
}
